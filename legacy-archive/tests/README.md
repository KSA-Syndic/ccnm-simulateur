...
Ce dossier (`legacy-archive/tests/`) contient les tests **oracle** du bundle legacy (Vitest, chemins relatifs vers les modules sous `legacy-archive/`).

## Structure

```
legacy-archive/tests/
├── setup.js              # Configuration globale Vitest (legacy)
...
```bash
npx vitest run legacy-archive/tests/unit/ClassificationEngine.test.js
```
...
// depuis legacy-archive/tests/unit/ : remonter vers legacy-archive/<module>.js
import { maFonction } from '../../utils/formatters.js';
