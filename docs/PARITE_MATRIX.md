...
> Dernière mise à jour : 2026-05-27 — documentation alignée sur `src/` (Vue 3) ; `legacy-archive/` **optionnel / supprimable**.

## Lecture de la matrice

| Colonne                  | Sens                                                                                                                                                    |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Périmètre historique** | Trace de l’ancien code JS (`legacy-archive/`, supprimable).                                                                                             |
| **Cible Vue / TS**       | **Code et chemins actifs** sous `src/`.                                                                                                                 |
| **Preuve / tests**       | Tests **à exécuter sans legacy** : `src/**/__tests__`, `tests/**`, `e2e/**`. Les fichiers `legacy-archive/tests/**` ne sont que des oracles optionnels. |
...
| D3.04 | tooltips primes équipe (archive)        | `buildPrimeConditionTooltip` + `builders.ts`, définitions accord | `src/domain/tooltip/builders.test.ts`, `accord-element-defs.test.ts`, profil Kuhn `tests/parity/*` (opt.) | `completed` |
...
| ID    | Périmètre historique          | Cible Vue / TS                                               | Preuve                                                                             | Statut      |
| ----- | ----------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ----------- |
| D4.01 | moteur rémunération (archive) | `engine.ts`, `compute.ts`, `aggregate.ts`, `smh.ts`          | `src/domain/remuneration/__tests__/engine.test.ts`, `tests/parity/*` (opt.)        | `completed` |
...
| D6.03 | Parité dual 5174 / 5173 (**opt-in**) | `e2e/parite-visuelle.spec.ts`, `DUAL_PARITE_E2E`, CI `dual-parity`                                         | Nécessite `legacy-archive/` + `npm run dual` — **non bloquant** pour la gate passe 2                                                                                           | `completed` |
| D6.04 | Parité visuelle pixel (**opt-in**)   | `e2e/parite-visuelle-pixels.spec.ts`, `comparePng.ts`                                                      | Idem D6.03 ; supprimable avec l’archive JS                                                                                                                                     | `completed` |
...
## Tests Vitest (référence sans legacy)

**Sans `legacy-archive/`** : `vitest.config.js` peut ne garder que :

- `src/**/*.test.ts`, `src/**/*.spec.ts`
- `tests/**/*.test.ts`, `tests/**/*.spec.ts`

**Avec oracle** (optionnel) : ajouter `legacy-archive/tests/**/*.test.js` et `tests/parity/remuneration-oracle.test.ts`.
